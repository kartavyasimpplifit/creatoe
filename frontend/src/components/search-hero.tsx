  ...
  return (
    <div className="min-h-[92vh] flex flex-col items-center justify-center px-6">
      ...
      <div className="mb-10">
        ...
        <div className="text-[11px] text-[var(--text-muted)] tracking-[0.2em] uppercase mt-2 ml-0.5">by Suggest</div>
      </div>

      <p className="text-[15px] text-[var(--text-secondary)] max-w-md text-center mb-8 leading-relaxed">
        AI-powered creator intelligence for commerce.
      ...
      <div className="flex items-center bg-[var(--bg-card)] border border-[var(--border)] rounded-full p-1 mb-6">
        {([
          { key: "product" as Mode, label: "Product Match", icon: "🔗" },
          { key: "creators" as Mode, label: "Creators", icon: "👥" },
          { key: "videos" as Mode, label: "Videos", icon: "▶" },
        ]).map(m => (
          <button key={m.key} onClick={() => { setMode(m.key); setInput(""); }}
            className={`px-4 py-2 rounded-full text-xs font-medium transition-all flex items-center gap-1.5 ${
              mode === m.key ? "bg-[var(--accent)] text-white shadow-lg shadow-[var(--accent)]/20" : "text-[var(--text-muted)] hover:text-[var(--text)]"
            }`}>
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-xl mb-4">
        <div className="relative group">
          <div className="absolute -inset-0.5 bg-gradient-to-r from-[var(--accent)]/15 to-purple-500/15 rounded-2xl opacity-0 group-focus-within:opacity-100 blur-sm transition-opacity" />
          <div className="relative flex items-center bg-[var(--bg-card)] border border-[var(--border)] rounded-2xl focus-within:border-[var(--accent)]/40 transition-all">
            <div className="pl-4 pr-2 text-[var(--text-muted)]">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
            </div>
            <input type="text" value={input}
              onChange={e => { setInput(e.target.value); if (e.target.value.includes("flipkart.com") || e.target.value.includes("amazon.in") || e.target.value.includes("http")) setMode("product"); }}
              placeholder={mode === "product" ? "Paste a Flipkart or Amazon link..." : mode === "creators" ? "Describe the creator you need..." : "What videos are you looking for..."}
              className="flex-1 bg-transparent py-3.5 px-2 text-sm text-[var(--text)] placeholder-[var(--text-muted)] outline-none" />
            <button type="submit" disabled={!input.trim()}
              className="m-1.5 px-5 py-2 bg-[var(--accent)] hover:bg-[var(--accent-hover)] disabled:opacity-15 text-white text-sm font-semibold rounded-xl transition-all">
      ...
      {suggestions.length > 0 && (
        <div className="flex flex-wrap justify-center gap-2 max-w-xl mb-6 fade-in">
          {suggestions.map(q => (
            <button key={q} onClick={() => { setInput(q); onAISearch(q, mode); setShowLogo(false); }}
      ...
      {mode === "product" && (
        <button onClick={() => { const u = "https://www.flipkart.com/realme-narzo-70-5g-forest-green-128-gb/p/itmb71c5e63b49db"; setInput(u); onProductSearch(u); setShowLogo(false); }}
        ...
        <div className="flex gap-10 mt-14">
          {[
            ...
            <div key={s.label} className="text-center">
              <div className="text-xl font-bold text-[var(--text)] font-[family-name:var(--font-mono)]">{s.value}</div>
              <div className="text-[9px] text-[var(--text-muted)] uppercase tracking-[0.2em] mt-1 font-medium">{s.label}</div>
            </div>
          ))}
        </div>
      )}
